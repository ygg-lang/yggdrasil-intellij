...
public class YggdrasilParser implements PsiParser, LightPsiParser {
  ...
  /* ********************************************************** */
  // expression-group | identifier | escape | string | regex | function-call | number
  ...
  /* ********************************************************** */
  // BRACE_L (key COLON)? identifier BRACE_R
  ...
  public static boolean define_class(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "define_class")) return false;
    boolean r, p;
    Marker m = enter_section_(b, l, _NONE_, DEFINE_CLASS, "<define class>");
    r = annotations(b, l + 1);
    r = r && consumeToken(b, KW_CLASS);
    p = r; // pin = 2
    r = r && report_error_(b, identifier(b, l + 1));
    r = p && report_error_(b, define_class_3(b, l + 1)) && r;
    r = p && report_error_(b, define_class_4(b, l + 1)) && r;
  ...
  // class-cast?
  private static boolean define_class_3(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "define_class_3")) return false;
    class_cast(b, l + 1);
    return true;
  }

  // OP_REMARK?
  private static boolean define_class_4(PsiBuilder b, int l) {
    if (!recursion_guard_(b, l, "define_class_4")) return false;
  ...
  /* ********************************************************** */
  // annotations KW_GROUP identifier? group-body
  ...
  private static boolean number_1(PsiBuilder b, int l) {
    ...
    /* ********************************************************** */
    // annotations KW_Operators identifier? group-body
    public static boolean operators(PsiBuilder b, int l) {
        ...
        r = annotations(b, l + 1);
        r = r && consumeToken(b, KW_OPERATORS);
        p = r; // pin = 2
        r = r && report_error_(b, operators_2(b, l + 1));
        r = p && group_body(b, l + 1) && r;
        exit_section_(b, l, m, r, p, null);
        return r || p;
    }
    ...
    // identifier?
    private static boolean operators_2(PsiBuilder b, int l) {
        if (!recursion_guard_(b, l, "operators_2")) return false;
        identifier(b, l + 1);
        return true;
    }
  ...
  /* ********************************************************** */
  // annotations KW_USING identifier (DOT (OP_MANY | using-body))?
